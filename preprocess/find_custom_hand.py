...
def find_boxes(bbox_file, filter_field=None):  
    """
    ...
    for key in ['hand', 'obj']:
        ...
        gt_boxes.append(bbox[key] )
        is_object.append(key == 'obj')
        hand_side.append(side)
        if key == 'obj':
            contact_bbox.append(bbox['obj'])
...
def find_corsp_hand(seqname):
    json_file = osp.join(data_dir, seqname, 'bbox.json')
    gt_boxes, is_object, hand_side, contact_box = find_boxes(json_file)
